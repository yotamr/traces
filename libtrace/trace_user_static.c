...
unsigned int __local_obj_key __attribute__ ((visibility ("internal")));
...
static int TRACE__register_object(const char *object_name)
{
    __local_obj_key = trace_allocate_obj_key();
    map_static_log_data(object_name, __local_obj_key);
    return 0;
}

static void TRACE_static__init(void) __attribute__((constructor));
static void TRACE_static__init(void)
{
    char buffer_name[512];
    get_exec_name(buffer_name, sizeof(buffer_name));
    TRACE__register_object(buffer_name);
}
